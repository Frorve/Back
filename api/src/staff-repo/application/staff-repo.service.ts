...
export class StaffRepoService {
  ...
  async update(
    staffId: number,
    repoId: number,
    staffRepoDto: StaffRepoDto
  ): Promise<StaffRepo> {
    await this.staffRepoRepository.update(
      { staff_id: staffId, repo_id: repoId },
      staffRepoDto
    );
  ...
  async findUserById(userId: number): Promise<{ username: string } | null> {
    const user = await this.staffRepoRepository.query(
      `SELECT nombre FROM staff_repo WHERE staff_id = $1`,
      [userId]
    );
    return user[0] || null;
  }
